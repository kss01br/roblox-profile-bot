...
async function buildRankingEmbeds(top = 10) {
  ...
  if (!universeIds.length) {
    throw new Error("Nenhum universeId válido foi encontrado em maps.js.");
  }
...
async function sendOrUpdateRanking(client, channelId, top = 10) {
  if (!channelId) {
    console.log("RANKING_CHANNEL_ID não configurado.");
    return;
  }
  ...
  if (!channel || !channel.isTextBased()) {
    console.log("Canal de ranking inválido.");
    return;
  }

  const { embeds } = await buildRankingEmbeds(top);

  if (rankingMessageId) {
    const oldMessage = await channel.messages.fetch(rankingMessageId).catch(() => null);

    if (oldMessage) {
      await oldMessage.edit({ embeds });
      return;
    }
  }

  const sentMessage = await channel.send({ embeds });
  rankingMessageId = sentMessage.id;
}
